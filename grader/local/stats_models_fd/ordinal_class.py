...
parser.add_argument('--with_loss_weights', action='store_true')
...
with_loss_weights = args.with_loss_weights
...
if with_loss_weights:
    from sklearn.utils.class_weight import compute_class_weight 
    classes = np.array([l for l in range(num_labels)]) 
...
# model training
model = OrdClass()
model.class_weight_ = class_weights
